...
interface LatestTreatmentsCardProps {
	className?: string;
	featuredTreatments: PostWithMemberAndCategory[];
}

export function LatestTreatmentsCard({
	className,
	featuredTreatments,
}: LatestTreatmentsCardProps) {
	return (
		// Use Card component with bg-muted
		<Card className={cn("bg-muted border-muted", className)}>
			<CardHeader className='flex flex-row items-center justify-between pb-4'>
				<CardTitle className='text-xl font-bold'>
					Câu chuyện bác sĩ nổi bật
				</CardTitle>
				<Link
					href='/case-studies'
			...
			<CardContent>
				<div className='space-y-3'>
					{featuredTreatments.length > 0 ? (
						...
						<p className='text-sm text-muted-foreground'>
							Không có y án nào được đề xuất
						</p>
					)}
				</div>
			</CardContent>
